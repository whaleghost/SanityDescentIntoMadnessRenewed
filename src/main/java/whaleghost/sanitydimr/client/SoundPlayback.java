...
import whaleghost.sanitydimr.sound.HeartbeatSoundInstance;
import whaleghost.sanitydimr.sound.InsanitySoundInstance;
...
public class SoundPlayback
{
    ...
    private static BlockPos pickFakeStepPos(LocalPlayer player)
    {
        ...
        Vec3 randomDir = new Vec3(
                (RAND.nextFloat() * 2f - 1f) * 3f,
                0,
                (RAND.nextFloat() * 2f - 1f) * 3f
        );
        Vec3 pos = player.position().add(lookAngle.multiply(-2f, -2f, -2f)).add(randomDir);
